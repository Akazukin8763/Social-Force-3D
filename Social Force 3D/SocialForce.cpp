...
Pedestrian* SocialForce::GeneraePedestrianByType(int type) {
	...
	float range = 10.0f * halfGroundSize / 70.0f;

	Pedestrian* pedestrian;

	int random, randomTemp2;
	int randomTemp = Utils::RandomFloat(-range, range);
	...
	if (type == 0) {
		pedestrian = new Pedestrian(glm::vec3(start, 0.0f, mid + randomTemp));
		pedestrian->AddCheckpoint(Checkpoint{ glm::vec3(mid,   0.0f, mid + randomTemp), CHECKPOINT_RADIUS });
	}
	else if (type == 1) {
		pedestrian = new Pedestrian(glm::vec3(mid + randomTemp, 0.0f, start));
		pedestrian->AddCheckpoint(Checkpoint{ glm::vec3(mid + randomTemp, 0.0f, mid), CHECKPOINT_RADIUS });
	}
	else if (type == 2) {
		pedestrian = new Pedestrian(glm::vec3(-mid + randomTemp, 0.0f, start));
		pedestrian->AddCheckpoint(Checkpoint{ glm::vec3(-mid + randomTemp, 0.0f, mid), CHECKPOINT_RADIUS });
	...
	randomTemp2 = Utils::RandomFloat(-range, range);
	randomTemp = Utils::RandomInt(0, 2);
	...
	if (random == 0) {
		pedestrian->AddCheckpoint(Checkpoint{ glm::vec3(mid + randomTemp2, 0.0f, mid + randomTemp2), CHECKPOINT_RADIUS });
